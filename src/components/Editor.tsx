...
import { CheckCircle, MagnifyingGlass, Plus, Sparkle } from "@phosphor-icons/react";
...
export default function Editor() {
  ...
  return (
    <div className="flex flex-1 overflow-hidden flex-col sm:flex-row bg-background">
      {/* Editor */}
      <div className="flex flex-col flex-1 min-w-0">
        <div className="flex items-center justify-between px-4 py-2.5 bg-surface border-b border-separator">
          <span className="text-xs font-medium text-muted">{t("editor.title")}</span>
          <div className="flex items-center gap-1">
            <Button variant="ghost" size="sm" onPress={handleAddSample}>
              <Plus size={13} weight="bold" />
              {t("editor.addSample")}
            </Button>
            <Button variant="ghost" size="sm" onPress={handleLoadSample}>
        ...
        <textarea
          ref={textareaRef}
          className="flex-1 p-5 text-sm font-mono resize-none bg-surface outline-none leading-7 text-foreground placeholder:text-muted/40 selection:bg-accent/15"
          value={rawText}
      ...
      {/* Divider */}
      <div className="w-px bg-separator shrink-0 hidden sm:block" />

      {/* Preview */}
      <div className="flex flex-col flex-1 min-w-0 bg-surface-secondary/50 overflow-hidden">
        {/* Header */}
        <div className="px-4 py-2.5 border-b border-separator flex items-center gap-2 flex-wrap">
          <div className="relative flex-1 min-w-30">
            ...
            <Input className="pl-8 text-xs" placeholder={t("editor.searchPlaceholder")} value={searchPreview} onChange={(e) => setSearchPreview(e.target.value)} fullWidth />
          </div>
          <div className="flex items-center gap-1">
            <Input className="w-24 text-center text-xs" placeholder={t("editor.gotoPlaceholder")} value={gotoPreview} onChange={(e) => setGotoPreview(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleGoToQuestion()} />
            <Button size="sm" variant="secondary" onPress={handleGoToQuestion}>
              {t("editor.find")}
            </Button>
          </div>
          {preview.length > 0 && (
            <span className="flex items-center gap-1 text-xs text-success font-medium ml-auto font-mono">
        ...
        <div ref={previewRef} className="flex-1 overflow-y-auto p-4 flex flex-col gap-2">
          {filteredPreview.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full gap-3 text-muted/40">
            ...
            filteredPreview.map((q) => (
              <div key={q.id} id={`preview-q-${q.id}`} className="bg-surface border border-border rounded-lg p-3 text-sm scroll-mt-4">
                <p className="font-medium mb-2 leading-snug text-foreground whitespace-pre-line">
